...
# إدخال آخر كود تم الوصول إليه
last_code_input = st.text_input("أدخل آخر كود تم الوصول إليه (اختياري):", placeholder="مثال: 57C أو 100Z أو 1AA")

def get_letter_code(index):
    """توليد تسلسل الحروف: A-Z, AA, AB, ..."""
...
def parse_last_code(code):
    """تحويل آخر كود إلى عدّاد رقمي"""
    if not code:
        return 0  # البدء من البداية
    match = re.match(r"(\d+)([A-Z]+)$", code.strip().upper())
    if not match:
        return 0
    ...
    letter_index -= 1
    return letter_index * 100 + number  # تحويل الكود إلى رقم متسلسل عالمي

# حساب بداية الترقيم
global_counter = parse_last_code(last_code_input)
