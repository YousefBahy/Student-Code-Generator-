...
def parse_last_code(code):
    """تحليل الكود الأخير (يدعم أرقام عربية أو إنجليزية + ترتيب عكسي أو مباشر)"""
    if not code:
        return 0
    code = convert_arabic_digits_to_english(code)
    code = code.strip().replace(" ", "").upper()

    # التقاط أي ترتيب: أرقام ثم حروف أو العكس
    ...
    if match.lastindex == 2:
        if match.group(1).isalpha():
            ...
            number, letters = int(match.group(1)), match.group(2)
    else:
        return 0
