...
for i in range(num_committees):
    with st.expander(f"🧮 اللجنة رقم {i+1}"):
        ...
        # إذا ترك المستخدم الاسم فارغًا، نستخدم اسمًا افتراضيًا "لجنة X"
        if not name.strip():
            name = f"لجنة {i+1}"
...
if st.button("🔢 توليد الأكواد"):
    ...
    for committee in committees:
        ...
        if committee_codes:
            ...
            current_letter = current_start[len(str(int(current_start[:-1]))):]
            for i in range(1, len(committee_codes)):
                next_code = committee_codes[i]
                next_letter = next_code[len(str(int(next_code[:-1]))):]
